# apps/tavern/h5-weapp — 微信小程序 + H5

## OVERVIEW
Taro 4.x 跨平台应用 (React 18 + Zustand 5 + TypeScript + Sass)，AI-Tavern 角色聊天应用。支持 WeChat 小程序和 H5 浏览器双平台。角色市场浏览、SSE 流式聊天、角色卡创建与管理、自定义人设。

## STRUCTURE
```
apps/tavern/h5-weapp/
├── src/
...
| 应用配置 | `src/app.config.ts` | 小程序/H5 双平台配置 |
| CSS 变量 | `src/app.scss` | 暗色主题/紫色主色 #8B5CF6 |
| SSE 流式 | `src/hooks/useSSE.ts` | EventSource 封装/断线重连 |

## CONVENTIONS
- Taro 4.x API (4.0.13)，构建命令 `taro build --type weapp` / `taro build --type h5`
...
- **源目录共用于双平台**: `src/` 同时提供 weapp 和 h5 构建
...
- **authStore**: 使用 `@tarojs/taro` Storage API，双平台兼容
- **H5 构建**: 在 `apps/tavern/` 下安装依赖后，运行 `npm run dev:h5` 即可
