...
type SortType = 'latest' | 'hot' | 'most_liked'
...
export default function MarketPage() {
  ...
  const fetchCarousel = useCallback(async () => {
    try {
      const res = await marketService.featured() as { data: CarouselItem[] }
  ...
  const fetchTags = useCallback(async () => {
    try {
      const res = await marketService.tags() as { data: Tag[] }
      setTags(res.data ?? [])
    } catch {
      // 静默处理
    }
  ...
  const fetchCharacters = useCallback(async (pageNum: number, isRefresh = false) => {
    try {
      ...
      const res = await marketService.list(params) as { data: CharacterCardType[], hasMore: boolean }
      if (isRefresh) {
  ...
  const fetchSearch = useCallback(async (query: string, pageNum: number, isRefresh = false) => {
    try {
      const res = await marketService.search(query, pageNum) as { data: CharacterCardType[], hasMore: boolean }
      if (isRefresh) {
        setCharacters(res.data ?? [])
      } else {
        setCharacters(prev => [...prev, ...(res.data ?? [])])
  ...
  return (
    <View className='page-market'>
            ...
            {carouselItems.map((item) => (
              ...
              >
                ...
                <View className='market-carousel-overlay'>
                  ...
                  <Text className='market-carousel-likes'>❤️ {item.likes}</Text>
                </View>
              </View>
            ))}
          </ScrollView>
      ...
      {/* 标签栏 */}
      {tags.length > 0 && (
        <ScrollView scrollX className='market-tags'>
      ...
      <View className='market-sort'>
        ...
        <View
          className={`market-sort-item ${sortType === 'hot' ? 'active' : ''}`}
          onClick={() => handleSortChange('hot')}
        >
          <Text>最热</Text>
        </View>
        <View
          className={`market-sort-item ${sortType === 'most_liked' ? 'active' : ''}`}
