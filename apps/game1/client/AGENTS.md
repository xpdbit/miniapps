...
## OVERVIEW
Taro 4.x + React 18 微信小程序 + H5 网页，挂机放置游戏。纯 TS 游戏逻辑引擎 + Zustand 状态管理 + JSON 数据驱动配置。当前 Phase 1 — P2 开发阶段。
支持双平台构建：微信小程序 (weapp) 和 H5 网页。
...
npm run dev:h5           # H5 网页开发模式

# 构建
npm run build:weapp      # 微信小程序开发构建
npm run build:weapp:prod # 微信小程序生产构建
npm run build:h5         # H5 网页开发构建
npm run build:h5:prod    # H5 网页生产构建
