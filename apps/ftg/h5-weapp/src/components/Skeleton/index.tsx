...
const Skeleton: FC<SkeletonProps> = ({ type = 'card', count = 1, style }) => {
  const items = Array.from({ length: count });

  if (type === 'grid') {
    return (
      <View className='skeleton-grid' style={style}>
        {items.map((_, i) => (
  ...
  if (type === 'card') {
    return (
      <View className='skeleton-card-list' style={style}>
        {items.map((_, i) => (
          <View key={i} className='skeleton-card'>
            <View className='skeleton-img skeleton-pulse' />
  ...
  if (type === 'list') {
    return (
      <View className='skeleton-list' style={style}>
        {items.map((_, i) => (
  ...
  // type === 'image'
  return (
    <View className='skeleton-image-wrap' style={style}>
      {items.map((_, i) => (
        <View key={i} className='skeleton-image-block skeleton-pulse'>
          <View className='skeleton-image-icon'>🖼️</View>
        </View>
      ))}
    </View>
  );
