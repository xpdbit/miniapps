...
import { fetchUserStatsHttp, fetchRecordsListHttp } from '@/services/userService';
...
export default function StatsPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // 并行加载用户统计与记录数据（使用 HTTP API 替代 CloudBase）
      const [userStatsData, recordsResult] = await Promise.all([
        fetchUserStatsHttp(token),
        fetchRecordsListHttp(token, { page: 1, limit: 999 }),
      ]);

      setStats(userStatsData);
      const records = recordsResult.list;
