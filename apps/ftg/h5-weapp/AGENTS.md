# apps/ftg/h5-weapp — 微信小程序 (Taro)

## OVERVIEW
Taro 4.x 微信小程序 + H5 (React 18 + TypeScript + Sass)，AI 图片识别食材 → Canvas 合成主题图片。
...
## CONVENTIONS
- Taro 4.x API，构建命令 `taro build --type weapp`（小程序）或 `taro build --type h5`（H5）
...
npm run build:h5:prod     # H5 生产+压缩构建
...
- **H5 兼容**: `Taro.login()` 在 H5 环境下会失败并自动降级为 Mock 登录（服务端需支持 `dev_` 前缀 code）
- **H5 认证降级**: 微信小程序使用 wx.login，H5 使用 Mock code (`dev_` 前缀) 登录，确保服务端已配置对应路由
