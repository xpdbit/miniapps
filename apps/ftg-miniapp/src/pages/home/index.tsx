...
import { fetchTodayStatus, fetchCheckinStreak } from '@/services/checkinService';
import { foodRecordDAL } from '@/services/db';
...
export default function HomePage() {
  ...
  // ============================================================
  // 数据加载（HTTP API + CloudBase 降级）
  // ============================================================
  const loadData = useCallback(async () => {
    setDataLoading(true);
    try {
      // 采用 Promise.all 并发获取打卡状态和记录数据
      const [status, streak, records] = await Promise.all([
        fetchTodayStatus().catch(() => ({ checkedIn: false, streakCount: 0 })),
        fetchCheckinStreak().catch(() => 0),
        (async () => {
          const openid = authUser?.openid ?? '';
          if (!openid) return { list: [] as FoodRecord[] };
          try {
            return await foodRecordDAL.getByOpenId(openid, { page: 1, pageSize: 3 });
          } catch {
            return { list: [] as FoodRecord[] };
          }
        })(),
      ]);

      setCheckedIn(status.checkedIn);
      setStreakCount(status.streakCount || streak);
