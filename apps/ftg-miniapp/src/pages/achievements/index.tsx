  ...
  const loadProgress = useCallback(async () => {
    try {
      ...
      interface ServerAchievement {
        achievementId: string; progress: number; isUnlocked: boolean; unlockedAt: string;
      }

      if (token) {
        const res = await httpClient.get<ApiResponse<ServerAchievement[]>>('/achievements/my', token);
        if (res.success && Array.isArray(res.data)) {
          const map: Record<string, UserAchievement> = {};
          for (const sa of res.data) {
            map[sa.achievementId] = {
              openid: '',
