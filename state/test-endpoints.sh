...
docker exec docker-nginx curl -s -o /dev/null -w "  /v1/admin/dashboard/stats: %{http_code}\n" http://tavern-server:3002/v1/admin/dashboard/stats -H 'Authorization: Bearer tavern_admin' --connect-timeout 5
docker exec docker-nginx curl -s -o /dev/null -w "  /v1/admin/model-stats: %{http_code}\n" http://tavern-server:3002/v1/admin/model-stats -H 'Authorization: Bearer tavern_admin' --connect-timeout 5

echo ""
echo "=== Via Dashboard Proxy ==="
