...
router.post('/characters', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    console.error('[Admin] POST /characters failed:', err)
...
router.put('/characters/:id', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const card = await prisma.characterCard.update({
...
router.delete('/characters/:id', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const card = await prisma.characterCard.findUnique({ where: { id: req.params.id } })
    ...
    await prisma.$transaction([
      prisma.characterCardLike.deleteMany({ where: { cardId: req.params.id } }),
      prisma.characterCardFav.deleteMany({ where: { cardId: req.params.id } }),
      prisma.chatMessage.deleteMany({ where: { session: { characterId: req.params.id } } }),
      prisma.chatSession.deleteMany({ where: { characterId: req.params.id } }),
      prisma.characterCard.delete({ where: { id: req.params.id } }),
    ])
...
router.get('/chats/:chatId/messages', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const [items, total] = await Promise.all([
      ...
      prisma.chatMessage.count({ where: { sessionId: req.params.chatId } }),
    ])
