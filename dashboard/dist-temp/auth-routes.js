...
// ─── 工具函数 ────────────────────────────────────────────────
function signAccessToken(userUuid, role) {
    return jsonwebtoken_1.default.sign({ sub: userUuid, role }, JWT_SECRET, { expiresIn: ACCESS_TOKEN_EXPIRES });
}
    ...
    try {
        ...
        if (!matchedAuth) {
            ...
            for (const auth of allPasswordAuths) {
                const valid = await bcryptjs_1.default.compare(credential, auth.credential).catch(() => false);
                if (valid) {
                    matchedAuth = auth;
                    break;
                }
        ...
        const passwordValid = await bcryptjs_1.default.compare(password, user.auths[0].credential);
        if (!passwordValid) {
            res.status(401).json({ code: 401, message: '用户名或密码错误' });
            return;
        }
...
authRouter.post('/bind', async (req, res) => {
    try {
        ...
        // 检查凭证是否已被绑定
        const existing = await prisma.userAuth.findFirst({
            where: { authType: type, credential },
        });
        if (existing) {
        ...
        if (type === 'password') {
            finalCredential = await bcryptjs_1.default.hash(credential, BCRYPT_ROUNDS);
        }
