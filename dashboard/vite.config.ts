...
export default defineConfig(({ mode }) => ({
  ...
  server: {
    ...
    proxy: {
      ...
      // Admin API 代理 — 必须放在 /api/v1 之前
      // 匹配 /api/v1/admin/*，转发到 Dashboard Admin API (3001)
      '/api/v1/admin': {
      ...
      // 匹配 /api/v1/tavern/*，rewrite 为 /api/v1/* 后转发（与生产 Nginx 路由对齐）
      '/api/v1/tavern': {
        target: 'http://localhost:3002',
        changeOrigin: true,
        rewrite: (path) => path.replace(/^\/api\/v1\/tavern/, '/api/v1'),
      },
      ...
      // 主 API 代理 — /api/v1/* 转发到 ftg-server (3000)
      // 必须放在所有更具体的 /api/v1/* 规则之后
      '/api/v1': {
