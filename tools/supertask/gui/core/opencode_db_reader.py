...
class OpencodeDBReader:
    ...
    def _batch_fill_token_stats(self, conn: sqlite3.Connection, sessions: List[SessionRecord]):
        """批量从 message 表填充 token/cost 统计数据"""
        if not sessions:
            return

        # 用 IN 查询所有 session 的消息
        for sess in sessions:
            try:
                ...
                for row in cursor.fetchall():
                    ...
                    if parsed["model_id"]:
                        model_set.add(parsed["model_id"])
                    if parsed["provider_id"]:
                        provider_set.add(parsed["provider_id"])
                    if parsed["agent_name"]:
                        agent_name_set.add(parsed["agent_name"])

                sess.model_id = ", ".join(sorted(model_set)) if model_set else ""
                sess.provider_id = ", ".join(sorted(provider_set)) if provider_set else ""
                sess.agent_name = ", ".join(sorted(agent_name_set)) if agent_name_set else ""
                sess.message_count = msg_count
        ...
        def _build_tree(session_id: str, depth: int = 0) -> List[AgentSessionRecord]:
            ...
            try:
                ...
                for row in cursor.fetchall():
                    ...
                    rec.agent_type = self._extract_agent_type(row["title"] or "")
                    # 填充 token 统计
    ...
    @staticmethod
    def _extract_agent_type(title: str) -> str:
    ...
    def get_agent_type_stats(self, days: int = 30) -> List[dict]:
        ...
        for sess in sessions:
            ...
            atype = self._extract_agent_type(sess.title)
            if not atype:
                atype = "unknown"
