...
class SessionCardWidget(QFrame):
    ...
    def __init__(self, session: dict, parent: QWidget | None = None):
        ...
        dot_label = QLabel(f"{_dot} {status_text}")
        dot_label.setStyleSheet(f"color: {self._status_color}; font-size: 13px; font-weight: bold; "
        ...
        dur_label.setStyleSheet(f"color: {TEXT_SECONDARY}; font-size: 11px; "
                                f"background: transparent; border: none;")
        info_row.addWidget(dur_label)
        countdown_text = DataStore.fmt_idle_countdown(last_ts)
        countdown_label = QLabel(f"⏳ {countdown_text}")
        countdown_label.setStyleSheet(f"color: {TEXT_SECONDARY}; font-size: 11px; "
                                      f"background: transparent; border: none;")
        info_row.addWidget(countdown_label)
        info_row.addStretch()
...
class SessionCardView(QWidget):
    ...
    def set_sessions(self, sessions: list[dict]) -> None:
        ...
        cols, rows = _calc_grid(len(sessions))
...
class OverviewInterface(QWidget):
    ...
    def _update_session_table(self) -> None:
        ...
        for i, sess in enumerate(self._sessions):
            ...
            # ── 列 4：检测倒计时（每秒刷新）──
            countdown_item = QTableWidgetItem(DataStore.fmt_idle_countdown(last_ts))
    ...
    def _update_elapsed_column(self) -> None:
        ...
        for i in range(table.rowCount()):
            ...
            if countdown_item:
                countdown_item.setText(DataStore.fmt_idle_countdown(last_ts))
