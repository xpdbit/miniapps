# -*- coding: utf-8 -*-
"""适配器：封装 OCE 的 opencode_db_reader.py。

从本地 core.opencode_db_reader 导入 OpencodeDBReader，
不再依赖已移除的 supertask 路径。
"""
...
class OceDBAdapter:
    ...
    def __init__(self, db_path: Optional[str] = None):
        self._reader: Optional[OpencodeDBReader] = None
    ...
    def _get_reader(self) -> Optional[OpencodeDBReader]:
        """安全获取 reader 引用。"""
    ...
    @property
    def reader(self) -> Optional[OpencodeDBReader]:
        """公开 reader 引用（供 DataStore worker 初始化）。"""
    ...
    def db_path(self) -> Optional[str]:
        ...
        return reader.db_path if reader else None
    ...
    def get_api_history(self, days: int = 7, search: str = "",
        ...
        try:
            # 按天数动态计算加载上限：
            # - 展示 50 条/页，只读前 N 页的量
            # - 无搜索/过滤时 ≈ days * 200，但有 +300 缓冲
            max_needed = (page + 2) * page_size  # 当前页 + 2 页缓冲
            limit = min(5000, max(200, max_needed, days * 300))
            records = reader.list_sessions(days=days, limit=limit)
            items = []
            for r in records:
