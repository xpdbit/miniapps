...
class LoopEngine:
        ...
        try:
            while not self._stop_requested:
                ...
                if result.success:
                    ...
                    # 通知监管 Agent 失败计数
                    if self._supervisor:
                        self._supervisor._check_failure_counter(self._consecutive_failures)
    ...
    def _supervisor_notify(self, category: str, message: str):
        """通知监管 Agent"""
        if self._supervisor:
            from .supervisor import AlertLevel
            self._supervisor._alert(AlertLevel.INFO, category, message)  # type: ignore
    ...
    def _parse_proposals(self, output: str) -> list[dict]:
        ...
        def _try_extract_list(data) -> list[dict] | None:
            """尝试从解析后的数据中提取列表"""
            if isinstance(data, list):
                return data
            if isinstance(data, dict):
                for key in ('proposals', 'items', 'tasks', 'suggestions', 'results'):
                    val = data.get(key)
        ...
        for block in re.findall(r'```(?:yaml|yml)\s*\n(.*?)```', output, re.DOTALL):
            try:
                result = _try_extract_list(yaml.safe_load(block.strip()))
        ...
        # 策略 3: 尝试从文本中提取列表/映射片段
        lines = output.split('\n')
        yaml_lines = []
        in_yaml = False
        for line in lines:
            stripped = line.strip()
            # 检测 YAML 列表项（- xxx）或映射项（key: value）
            if re.match(r'^(- |\w[\w-]*: )', stripped):
                in_yaml = True
            if in_yaml:
                yaml_lines.append(line)
            # blank line resets unless we already have content
            elif not stripped and yaml_lines:
                pass  # keep going — blank lines inside YAML are fine
            elif yaml_lines and not stripped.startswith('```'):
                # if we were collecting and hit a non-YAML line, stop
                if not stripped.startswith('#') and ':' not in stripped and not stripped.startswith('-'):
                    break

        if yaml_lines:
            yaml_str = '\n'.join(yaml_lines)
            try:
                result = _try_extract_list(yaml.safe_load(yaml_str))
                if result is not None:
                    return result
            except yaml.YAMLError:
                pass
        ...
        # 策略 4: 回退 — 尝试解析整个输出
        try:
            result = _try_extract_list(yaml.safe_load(output))
            if result is not None:
                return result
        except yaml.YAMLError:
            pass
