...
| `App` (Server) | 入口 | `servers/ftg-server/src/app.ts` | FTG 后端 Express 服务 |
| `App` (Game1 Server) | 入口 | `servers/game1-server/src/app.ts` | Game1 后端 Express 服务 |
| `App` (Tavern Server) | 入口 | `servers/tavern-server/src/app.ts` | Tavern 后端 Express 应用定义 |
| `index` (Tavern Server) | 入口 | `servers/tavern-server/src/index.ts` | Tavern 服务器启动监听 |
| `App` (Game1 MiniApp) | 入口 | `apps/game1-miniapp/src/app.tsx` | Game1 小程序入口 |
...
- ❌ **空 catch 块** — apps/ftg-miniapp/src/app.ts 有 4 个空 catch 块（line 55/94/117/131）
