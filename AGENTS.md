...
| 数据库 Schema | `prisma/schema.prisma` | 统一 Prisma Schema (14表) |
...
- ❌ **Tavern CI 路径过滤器错误** — `.github/workflows/ci.yml` 使用 `servers/tavern-server/**`，实际路径为 `apps/tavern/server/`，CI 永远不会触发
...
- ❌ **tavern-server 路径别名缺失** — 未配置 `@/*`，使用相对路径 import
- ❌ **tavern-server 超时配置缺失** — 未设置 keepAliveTimeout/headersTimeout/timeout
...
- **Dashboard Auth 初始化**: authStore 初始化时 `isAuthenticated` 同步设为 `!!getToken()`，`user` 为 `null`。`restoreSession()` 异步调用 `/admin/me` 获取用户信息。`ProtectedRoute` 订阅 `initialized` 标志，仅在 `restoreSession` 完成后才进行权限判断
