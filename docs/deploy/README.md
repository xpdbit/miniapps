...
*.mnapp.top → 47.94.108.150 (ECS)
  ...
  │                   mnapp.top                        │
  ├── /                       → Dashboard SPA          │
  ├── /api/ftl/api/v1/*      → ftg-server:3000        │
  ├── /api/ftl/health        → ftg-server:3000        │
  ├── /api/ftl/uploads/*     → ftg-server:3000        │
  ...
  └── /dashboard              → Admin dashboard        │
                                                       │
  game1.mnapp.top → Game1 SPA（占位）                  │
  tavern.mnapp.top → Tavern SPA（预留）                │
  ftl.mnapp.top   → 301 → mnapp.top（兼容旧链接）      │
└───────────────────────────────────────────────────────┘
```
